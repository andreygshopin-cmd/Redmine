...
def resolveSnapshotUseRiskPlan(planningProjects: list[dict[str, object]] | None) -> bool:
    projects = list(planningProjects or [])
    return bool(projects) and all(bool(project.get("use_risk_plan")) for project in projects)


def buildSnapshotDevelopmentTotalMetrics(
    summary: dict[str, object] | None,
    dynamicMetrics: dict[str, object] | None,
) -> dict[str, float]:
    summaryView = buildSnapshotSummaryView(summary)
    sourceMetrics = dict(dynamicMetrics or {})
    forecastTotal = float(sourceMetrics.get("development_total_forecast_hours") or 0)
...
def buildSnapshotIssueFiltersPayload(
    ...
    riskRemainingOp: str | None = None,
    riskRemainingValue: str | None = None,
    ...
    return {
        ...
        "risk_remaining_op": riskRemainingOp or "",
        "risk_remaining_value": riskRemainingValue or "",
        "spent_op": spentOp or "",
...
def buildBurndownFeatureGroups(issues: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for issue in issues:
        ...
        try:
            ...
            parentIssueId = 0
        if parentIssueId:
            parentIssueIds.add(parentIssueId)
    ...
    for issue in issues:
        ...
        hasChildren = issueId in parentIssueIds
...
def buildPlanningProjectsPanelHtml(
    ...
    planningUseRiskPlan = resolveSnapshotUseRiskPlan(planningProjects)
...
<body>
    ...
    snapshotDevelopmentTotals = buildSnapshotDevelopmentTotalMetrics(
        snapshotPayload.get("summary"),
        snapshotDynamicMetrics,
    )
  ...
  <link href="https://fonts.googleapis.com/css2?family=Golos+Text:wght@400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
      ...
      * {{ box-sizing: border-box; }}
      ...
      .summary-table .summary-group-head th {{ text-align: center; }}
  ...
  <body>
    <main>
      ...
      <div class="summary-block">
        <table class="summary-table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="summary-metric" id="summaryDevelopmentTotalForecast" data-hours="{snapshotDevelopmentTotals["development_forecast_total_hours"]}">{formatPageHours(snapshotDevelopmentTotals["development_forecast_total_hours"])}</td>
            </tr>
            <tr>
        ...
        <p class="summary-legend">Легенда: Прогноз на год = Прогноз - Факт (всего) + Факт (год).</p>
      </div>
      <div class="filter-reset-wrap">
    ...
    <script>
      ...
      function normalizeSnapshotFilters(filters) {{
        ...
        const numericMappings = [
          ...
          ["risk_remaining_op", "risk_remaining_value"],
      ...
      function hasActiveSnapshotFilters() {{
        ...
        return Boolean(
          ...
          filters.risk_remaining_op ||
          filters.risk_remaining_value ||
          filters.spent_op ||
          filters.spent_value ||
      ...
      function collectSnapshotFilters() {{
        return {{
          ...
          risk_remaining_op: String(document.querySelector('[data-filter-key="riskRemaining"][data-filter-role="op"]')?.value || ""),
          risk_remaining_value: normalizeNumericFilterValue(document.querySelector('[data-filter-key="riskRemaining"][data-filter-role="value"]')?.value || ""),
      ...
      function buildSnapshotQueryParams(page, includePagination = true) {{
        ...
        if (filters.risk_remaining_op) params.set("risk_remaining_op", filters.risk_remaining_op);
        if (filters.risk_remaining_value) params.set("risk_remaining_value", filters.risk_remaining_value);
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    th:first-child, td:first-child { padding-left: 6px; padding-right: 6px; }
    ...
    .compare-filter[data-compare-filter="deal_id"] { padding-left: 4px; padding-right: 4px; }
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    th input[data-filter="item_id"] { padding-left: 4px; padding-right: 4px; }
    .mono { font-family: "Cascadia Mono", Consolas, monospace; font-variant-numeric: tabular-nums; }
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .legend-swatch.changed { background: #d9fbff; }
...
__BITRIX_TOP_NAV__
    ...
    <section class="panel">
      ...
      <div class="highlight-legend" aria-label="Легенда подсветки">
        ...
        <span class="legend-item"><span class="legend-swatch changed"></span>Сумма изменилась</span>
...
def _resolveLatestSnapshotDateOnOrBefore(projectRedmineId: int, reportDate: date) -> str | None:
    for dateValue in listSnapshotDatesForProject(projectRedmineId):
        try:
            snapshotDate = date.fromisoformat(str(dateValue))
        except ValueError:
            continue
        if snapshotDate <= reportDate:
            return str(dateValue)
    return None


def _loadProjectsSummarySnapshotDevelopmentMetrics(
    projectRedmineId: int,
    reportDate: date,
    projectIdentifier: str,
) -> dict[str, float | str | bool] | None:
    snapshotDate = _resolveLatestSnapshotDateOnOrBefore(projectRedmineId, reportDate)
    if not snapshotDate:
        return None

    summaryPayload = getFilteredSnapshotIssuesForProjectByDate(
        projectRedmineId,
        snapshotDate,
        page=1,
        pageSize=1,
    )
    if not summaryPayload.get("snapshot_run"):
        return None

    projectIdentifierRaw = str(projectIdentifier or "").strip()
    if not projectIdentifierRaw:
        snapshotRun = summaryPayload.get("snapshot_run")
        if isinstance(snapshotRun, dict):
            projectIdentifierRaw = str(snapshotRun.get("project_identifier") or "").strip()

    dynamicPayload = enrichSnapshotPayloadWithFeatureForecasts(
        listFilteredSnapshotIssuesForProjectByDate(projectRedmineId, snapshotDate),
        projectIdentifierRaw,
    )
    dynamicSummary = buildSnapshotDynamicSummary(dynamicPayload.get("issues"))
    planningProjects = listPlanningProjectsByRedmineIdentifier(projectIdentifierRaw) if projectIdentifierRaw else []
    useRiskPlan = resolveSnapshotUseRiskPlan(planningProjects)
    dynamicMetrics = dynamicSummary["risk" if useRiskPlan else "default"]
    return {
        **buildSnapshotDevelopmentTotalMetrics(summaryPayload.get("summary"), dynamicMetrics),
        "snapshot_date": snapshotDate,
        "use_risk_plan": useRiskPlan,
    }
...
def _buildProjectsSummaryGroups(
    ...
    for group in groupedRows:
        ...
        if reportDate is not None:
            try:
                projectRedmineId = int(group.get("project_redmine_id") or 0)
            except (TypeError, ValueError):
                projectRedmineId = 0
            if projectRedmineId:
                snapshotMetrics = _loadProjectsSummarySnapshotDevelopmentMetrics(
                    projectRedmineId,
                    reportDate,
                    str(group.get("redmine_identifier") or ""),
                )
                if snapshotMetrics:
  ...
  <script>
    ...
    function renderProjectsSummaryTotals(groups) {{
      ...
      for (const group of groups) {{
        ...
        const forecastYearValue = Number(group?.development_forecast_year_hours);
        if (Number.isFinite(forecastYearValue)) {{
          forecastYearTotal += forecastYearValue;
...
def refreshBitrixCompaniesDictionary(limit: int = 20000) -> dict[str, object]:
    companiesPayload = fetchBitrixCompanies(
        portalUrl=config.bitrixPortalUrl,
        credential=config.bitrixCredential,
        limit=limit,
    )
    companies = [company for company in companiesPayload.get("companies", []) if isinstance(company, dict)]
    storeResult = upsertBitrixCompanies(companies)
    return {
        "auth_mode": companiesPayload.get("auth_mode"),
        "bitrix_total": companiesPayload.get("total"),
        **storeResult,
    }
...
def startBitrixSnapshotCapture(entities: str | None = Query(None)) -> dict[str, object]:
    ...
    try:
        ...
        detail = "Bitrix24 dictionaries request failed."
        if error.response is not None:
            detail = f"{detail} HTTP {error.response.status_code}."
        raise HTTPException(status_code=502, detail=detail) from error
    except RuntimeError as error:
...
def captureBitrixDealSnapshot() -> dict[str, object]:
    ...
    try:
        ...
        crmCompanyIds = [
            int(item.get("companyId") or item.get("COMPANY_ID") or 0)
            for item in [*leadItems, *invoiceItems]
            if isinstance(item, dict)
        ]
        invoiceCategoryIds = [
        ...
        leadDictionaries["company_names"] = crmCompanyNames
...
def enrichBitrixDealSnapshotResponsibleNames(payload: dict[str, object]) -> dict[str, object]:
    ...
    for deal in deals:
        if not isinstance(deal, dict):
            continue
        if not deal.get("assigned_by_name"):
            try:
    ...
    if missingCompanyIds:
        try:
            companyNames = getBitrixCompanyNamesByIds(missingCompanyIds)
        except Exception:
            companyNames = {}
...
def getProjectsSummaryApiV2(
    ...
    groups = _buildProjectsSummaryGroups(projects, reportDate)
    return {
        "projects": projects,
        "groups": groups,
        "report_date": reportDate.isoformat(),
...
def getProjectLatestSnapshotIssuesData(
    ...
    risk_remaining_op: str | None = Query(None),
    risk_remaining_value: str | None = Query(None),
    ...
    filters = buildSnapshotIssueFiltersPayload(
        ...
        riskRemainingOp=risk_remaining_op,
        riskRemainingValue=risk_remaining_value,
        spentOp=spent_op,
